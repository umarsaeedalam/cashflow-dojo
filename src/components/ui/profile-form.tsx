            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4">
                <FormField control={form.control} name="username" render={({ field }) => (
                    <FormItem className="grid grid-cols-4 items-center gap-4 gap-y-1">
                        ...
                        <FormControl className="col-span-3 max-[500px]:col-span-4">
                            <Input placeholder="Username" {...field} className="bg-primary border border-secondary text-secondary rounded-md max-[500px]:text-sm placeholder:text-secondary/50 !outline-none focus-visible:!outline-accent" />
                        </FormControl>
                ...
                <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem className="grid grid-cols-4 items-center gap-4 gap-y-1 outline-none focus-visible:!outline-accent">
                        ...
                        <FormControl className="col-span-3 max-[500px]:col-span-4">
                            <Input placeholder="Email" {...field} disabled className="bg-primary border border-secondary text-secondary rounded-md max-[500px]:text-sm placeholder:text-secondary/50 !outline-none focus-visible:!outline-accent" />
                        </FormControl>
                ...
                <FormField control={form.control} name="currency" render={({ field }) => (
                    <FormItem className="grid grid-cols-4 items-center gap-4 gap-y-1">
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                            <FormControl>
                                <SelectTrigger className="max-[500px]:text-sm col-span-3 max-[500px]:col-span-4 border rounded-md bg-primary hover:bg-secondary border-secondary text-secondary hover:text-neutral py-1 px-3 transition-colors ease-in-out duration-200 flex gap-4 focus:outline-none focus-visible:outline-accent">
                                    <SelectValue placeholder="Currency" />
                                </SelectTrigger>
                            </FormControl>
                            
                            <SelectContent className="rounded-md bg-secondary" onWheel={(e) => e.stopPropagation()}>
                ...
                <div className="flex justify-end">
                    <Button type="submit" disabled={isPending} className="bg-primary font-semibold text-accent border-2 rounded-lg mt-2 text-right border-accent hover:bg-accent hover:text-primary !outline-none focus-visible:!outline-secondary transition-all ease-in-out duration-200 transform active:scale-90">{isPending ? 'Updating...' : 'Save'}</Button>
                </div>
            </form>
        </Form>
    )
